...
public abstract class Pion implements Serializable {
    ...
    public ArrayList<Case> listeDeplacement;
    private Case c;
    ...
    public Pion(int vie, int force, int precision, int vitesse, int defense, int bonusChance, int portee, int mouvement, Case c) {
        ...
        listeDeplacement = new ArrayList<Case>();
    }
    ...
    public boolean deplacementPossible(Case c2) {
        ...
        return listeDeplacement.contains(c2);
    }
    ...
    public void deplacement(int distance) {
        ...
        listeOuverte.add(c);
        listeDeplacement.add(c);
        while (!listeOuverte.isEmpty()) {
            tmp = listeOuverte.remove(0);
            listeFerme.add(tmp);
            tmp2 = c.getPlateau().get(tmp.getLigne() + 1, tmp.getColonne());
            if (tmp2 != null) {
                System.out.println(tmp2.getObstacle());
            }
            if (tmp2 != null && !tmp2.isObstacle() && distanceManhattan(tmp2) <= distance
                    && !listeFerme.contains(tmp2) && !listeOuverte.contains(tmp2)) {
                listeDeplacement.add(tmp2);
            ...
            tmp2 = c.getPlateau().get(tmp.getLigne(), tmp.getColonne() + 1);
            if (tmp2 != null) {
                System.out.println(tmp2.getObstacle());
            }
            if (tmp2 != null && !tmp2.isObstacle() && distanceManhattan(tmp2) <= distance
